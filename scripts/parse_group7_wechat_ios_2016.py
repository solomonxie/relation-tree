...
def parse_ios_backup(backup_dir, out_conn):
    ...
    try:
        ...
        user_hashes = [
            row[0] for row in cursor.fetchall() if row[0] and len(row[0]) == 32
        ]
        conn.close()
    except Exception as e:
    ...
    for user_hash in user_hashes:
        ...
        if contact_db_path:
            ...
            try:
                ...
                for row in cursor.fetchall():
                    ...
                    if unick or uremark:
                        out_cursor.execute(
                            "UPDATE group7_raw_contacts SET nickname = ?, remark = ? WHERE username = ?",
                            (unick, uremark, uname),
                        )
            except Exception as e:
        ...
        if fts_db_path:
            ...
            try:
                ...
                logging.error(f"Error parsing FTS: {e}")
            conn.close()
        ...
        for pattern, mtype in media_patterns:
            ...
            for fid, rel in cursor.fetchall():
                ...
                try:
                    ...
                    logging.error(f"Error media {rel}: {e}")
        conn.close()
