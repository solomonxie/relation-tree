...
def cleanup():
    ...
    normalization_map = {
        ...
        "MINISTER OF DISCIPLESHIP AND COMMUNITY": ("FBC Discipleship & Community", "church group"),
    ...
    for gid, name, gtype in groups:
        ...
        if new_name:
            ...
            cursor.execute("SELECT id FROM groups WHERE name = ? AND id != ?", (new_name, gid))
            row = cursor.fetchone()
            if row:
                target_id = row[0]
                # Update person_groups to point to the existing normalized group
                cursor.execute("UPDATE person_groups SET group_id = ? WHERE group_id = ?", (target_id, gid))
                # Delete the redundant group
                cursor.execute("DELETE FROM groups WHERE id = ?", (gid,))
                print(f"Merged group '{name}' into '{new_name}'")
            else:
                # Rename the current group
                cursor.execute("UPDATE groups SET name = ?, type = ? WHERE id = ?", (new_name, new_type, gid))
