...
LLM_API_BASE = os.getenv("LLM_API_BASE", "http://localhost:11434/v1") # Default to Ollama
    ...
    for person in people_data:
        try:
            ...
            for pos in person.get("positions", []):
                ...
                cursor.execute("INSERT OR IGNORE INTO positions (name) VALUES (?)", (pos_name,))
            ...
            for edu in person.get("education", []):
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (person_id, edu.get("school"), edu.get("degree"), edu.get("major"), 
                      edu.get("start_date"), edu.get("end_date"), edu.get("notes")))
            ...
            for job in person.get("career", []):
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (person_id, job.get("company"), job.get("role"), 
                      job.get("start_date"), job.get("end_date"), job.get("notes")))
            ...
            for grp in person.get("groups", []):
                ...
                cursor.execute("SELECT id FROM groups WHERE name = ? AND type = ?", (group_name, grp.get("type")))
                row = cursor.fetchone()
                if row:
                    group_id = row[0]
                else:
                    cursor.execute("INSERT INTO groups (name, type) VALUES (?, ?)", (grp.get("name"), grp.get("type")))
    ...
    for rel in relationships_data:
        try:
            ...
            if p1_id and p2_id:
                ...
                conn.commit()
                print(f"Successfully inserted relationship: {p1_name} -> {p2_name} ({rel.get('type')})")
        except Exception as e:
            conn.rollback()
...
def main():
    parser = argparse.ArgumentParser(description="Extract people information using LLM and insert into SQLite.")
    parser.add_argument("file", help="Path to the text file containing people notes.")
    parser.add_argument("--chunk-size", type=int, default=4000, help="Maximum characters per LLM request.")
