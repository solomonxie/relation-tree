        ...
        try:
            ...
            for rid in redundant_ids:
                ...
                tables_with_person_id = [
                    "career", "contacts", "education", "financial_information", 
                    "media", "person_groups", "person_positions", "property"
                ]
                for table in tables_with_person_id:
                    cursor.execute(f"UPDATE {table} SET person_id = ? WHERE person_id = ?", (primary_id, rid))
                
                # 2. Update relationships (both person1_id and person2_id)
                cursor.execute("UPDATE relationships SET person1_id = ? WHERE person1_id = ?", (primary_id, rid))
                cursor.execute("UPDATE relationships SET person2_id = ? WHERE person2_id = ?", (primary_id, rid))
