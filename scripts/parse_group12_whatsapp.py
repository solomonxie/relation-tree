...
def main():
    ...
    cursor.execute(
        f"CREATE TABLE {table_name} (id INTEGER PRIMARY KEY AUTOINCREMENT, source_file TEXT, sender_name TEXT, sender_id TEXT, create_time INTEGER, content TEXT, platform TEXT, subfolder TEXT, msg_hash TEXT)"
    )
        ...
        with open(WHATSAPP_FILE, "r", encoding="utf-8", errors="replace") as f:
            ...
            for line in f:
                ...
                if match:
                    ...
                    try:
                        ts = int(
                            datetime.strptime(ts_str, "%d/%m/%Y, %H:%M:%S").timestamp()
                        )
                        current_msg = {
                            "sender_name": sender,
                            "sender_id": None,
            ...
            for m in messages:
                m_hash = compute_msg_hash(
                    m["sender_name"], m["create_time"], m["content"]
                )
                cursor.execute(
                    f"INSERT OR IGNORE INTO {table_name} (source_file, sender_name, sender_id, create_time, content, platform, subfolder, msg_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        WHATSAPP_FILE,
                        m["sender_name"],
                        m["sender_id"],
