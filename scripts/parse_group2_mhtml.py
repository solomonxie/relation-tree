...
def load_global_mappings():
    ...
    logging.info(
        f"Loaded {len(NAME_TO_ID)} name-to-ID mappings. Identified {len(GROUPS)} groups."
    )
...
def parse_file(filepath, cursor, seen_msgs):
    ...
    current_context_name = (
        ID_TO_NAME.get(current_context_id) if current_context_id else f_name
    )
    is_group_context = (
        current_context_id in GROUPS
        or current_context_name in GROUPS
        or "group" in filename.lower()
    )
    ...
    for row in rows:
        ...
        if obj_match:
            ...
            if not is_group_context:
                is_group_context = (
                    current_context_id in GROUPS or current_context_name in GROUPS
                )
            continue
        ...
        if msg_match:
            ...
            is_owner = (
                nickname == OWNER_NAME
                or "color:#42B475" in row
                or NAME_TO_ID.get(nickname) == OWNER_ID
            )
...
def main():
    ...
    for filepath in sorted(files):
        try:
            ...
            logging.info(
                f"Extracted {count} unique messages from {os.path.basename(filepath)}"
            )
