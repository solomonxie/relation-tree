...
def insert_person(cursor, person_data):
    ...
    if row:
        ...
        cursor.execute("SELECT display_name, nick_name, birthdate, notes FROM persons WHERE id = ?", (person_id,))
    ...
    for contact in person_data.get('contacts', []):
        if not contact.get('value'): continue
...
def extract_old_contacts():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT full_name, first_name, last_name, nickname, phones, emails, notes, birthday FROM contacts")
    ...
    for row in cursor.fetchall():
        full_name, first_name, last_name, nickname, phones_json, emails_json, notes, birthday = row
        ...
        contacts = []
        for p in phones: 
            if p: contacts.append({'type': 'phone', 'value': str(p)})
        for e in emails: 
            if e: contacts.append({'type': 'email', 'value': str(e)})
...
def extract_old_sms():
    ...
    for row in cursor.fetchall():
        name, phone = row
        if not name and not phone: continue
...
def extract_old_wechat():
    ...
    try:
        ...
        cursor.execute("SELECT username, nickname FROM contacts WHERE nickname != '' AND nickname IS NOT NULL")
    except sqlite3.Error:
        print(f"Table 'contacts' not found in {path}")
        return []
...
def extract_vcf():
    ...
    for vcf_file in vcf_files:
        ...
        for vcard in vcards:
            if 'END:VCARD' not in vcard: continue
            ...
            for line in lines:
                if line.startswith('FN:'):
                    ...
                    if not person['name']:
                        ...
                        full_name = " ".join([p.strip() for p in parts[::-1] if p.strip()])
                        person['name'] = full_name.strip()
                elif line.startswith('TEL'):
                    val = line.split(':')[-1].strip()
                    if val: person['contacts'].append({'type': 'phone', 'value': val})
                elif line.startswith('EMAIL'):
                    val = line.split(':')[-1].strip()
                    if val: person['contacts'].append({'type': 'email', 'value': val})
                elif line.startswith('ADR'):
                    val = line.split(':')[-1].strip().replace(';', ' ').strip()
                    if val: person['contacts'].append({'type': 'address', 'value': val})
...
def main():
    parser = argparse.ArgumentParser(description="Extract people information from various sqlite and vcf files in blobs/")
    parser.add_argument("--dry-run", action="store_true", help="Print extracted data instead of inserting into DB")
