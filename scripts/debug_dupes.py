...
def get_group4_messages():
    ...
    try:
        cursor.execute(
            "SELECT username, create_time, content FROM group4_raw_messages LIMIT 5"
        )
        ...
        for row in cursor.fetchall():
            ...
            ts_min = (ts // 60) * 60
            print(
                f"G4: {username} | {ts} ({datetime.fromtimestamp(ts)}) | {content[:30] if content else ''}"
            )
            h = hashlib.md5(
                f"{username}|{ts_min}|{content.strip() if content else ''}".encode(
                    "utf-8", errors="ignore"
                )
            ).hexdigest()
            msg_hashes.add(h)
...
def check_dupes():
    ...
    for line in lines:
        ...
        if match:
            ...
            h = hashlib.md5(
                f"{username}|{ts}|{msg_content.strip()}".encode(
                    "utf-8", errors="ignore"
                )
            ).hexdigest()
            if h in g4_hashes:
