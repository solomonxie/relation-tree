...
def get_group4_messages():
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            h = hashlib.md5(
                f"{username}|{ts_min}|{content}".encode("utf-8", errors="ignore")
            ).hexdigest()
            msg_hashes.add(h)
...
def check_dupes():
    ...
    for filename in txt_files:
        ...
        for line in lines:
            ...
            if match:
                ...
                h = hashlib.md5(
                    f"{username}|{ts}|{msg_content}".encode("utf-8", errors="ignore")
                ).hexdigest()
                if h in g4_hashes:
                    dupe_count += 1
                else:
    ...
    if total_txt_msgs > 0:
        print(
            f"Duplicate messages found in Group 4: {dupe_count} ({dupe_count / total_txt_msgs * 100:.1f}%)"
        )
