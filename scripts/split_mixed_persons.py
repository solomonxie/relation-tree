...
def split_mixed_person(person_id):
    ...
    cursor.execute("SELECT type, details FROM property WHERE person_id = ?", (person_id,))
    person['property'] = [dict(r) for r in cursor.fetchall()]
    
    cursor.execute("SELECT g.type, g.name, pg.role FROM person_groups pg JOIN groups g ON pg.group_id = g.id WHERE pg.person_id = ?", (person_id,))
    person['groups'] = [dict(r) for r in cursor.fetchall()]

    cursor.execute("SELECT pos.name, pp.organization, pp.notes FROM person_positions pp JOIN positions pos ON pp.position_id = pos.id WHERE pp.person_id = ?", (person_id,))
    ...
    payload = {
        "model": MODEL_NAME,
        "messages": [{"role": "user", "content": SPLIT_PROMPT.format(data=json.dumps(person))}],
        "temperature": 0.1,
...
def insert_person(cursor, person):
    ...
    for contact in person.get("contacts", []):
        cursor.execute("INSERT INTO contacts (person_id, type, value) VALUES (?, ?, ?)",
    ...
    for pos in person.get("positions", []):
        ...
        cursor.execute("INSERT INTO person_positions (person_id, position_id, organization, notes) VALUES (?, ?, ?, ?)",
                       (person_id, pos_id, pos.get("organization"), pos.get("notes")))
                       
    for grp in person.get("groups", []):
        name = grp.get("name")
        if not name: continue
        cursor.execute("SELECT id FROM groups WHERE name = ? AND type = ?", (name, grp.get("type")))
        row = cursor.fetchone()
        if row: group_id = row[0]
        else:
            cursor.execute("INSERT INTO groups (name, type) VALUES (?, ?)", (name, grp.get("type")))
            group_id = cursor.lastrowid
        cursor.execute("INSERT INTO person_groups (person_id, group_id, role) VALUES (?, ?, ?)",
                       (person_id, group_id, grp.get("role")))
                       
    for prop in person.get("property", []):
        cursor.execute("INSERT INTO property (person_id, type, details) VALUES (?, ?, ?)",
...
def main():
    ...
    cursor.execute("SELECT id FROM persons WHERE name LIKE '%&%' OR name LIKE '% and %'")
    ...
    for pid in ids:
        ...
        if new_people:
            try:
                ...
                cursor.execute("DELETE FROM person_positions WHERE person_id = ?", (old_id,))
                cursor.execute("DELETE FROM person_groups WHERE person_id = ?", (old_id,))
                cursor.execute("DELETE FROM property WHERE person_id = ?", (old_id,))
                cursor.execute("DELETE FROM education WHERE person_id = ?", (old_id,))
                cursor.execute("DELETE FROM financial_information WHERE person_id = ?", (old_id,))
